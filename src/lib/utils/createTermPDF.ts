...
import { env } from "@/env";
import { LGND_MARCAS_BASE64 } from "./lgnd-marcas-termo-base64";
import { LOGO_BASE64 } from "./logo-termo-base64";

function createBoxedTitle(
  doc: jsPDF,
  title: string | null,
  paragraphs: string[],
  y: number,
...
export async function createTermPDF(
  user: User,
  topNumber: number,
): Promise<string> {
  try {
    ...
    const config: PDFConfig = {
      ...
      titleFontSize: 14,
      bodyFontSize: 10,
      contentWidth: doc.internal.pageSize.getWidth() - 20 * 2,
    };

    const logoBackgroundImage = LOGO_BASE64;
    const legendaryPartners = LGND_MARCAS_BASE64;

    // Function to add background image to the current page
    const addBackgroundImage = () => {
      const x = (config.pageWidth - config.imgWidth) / 2;
      const y = (config.pageHeight - config.imgHeight) / 2;
      doc.addImage(
        logoBackgroundImage,
        "PNG",
        x,
    ...
    // Override addPage to include background image
    const originalAddPage = doc.addPage.bind(doc);
    doc.addPage = (...args: any[]) => {
      const newDoc = originalAddPage(...args);
      addBackgroundImage();
      return newDoc;
    };
    ...
    const checkPageBreak = (
      ...
      if (currentY + requiredHeight > config.pageHeight - config.margin) {
        doc.addPage();
        addBackgroundImage();
    ...
    doc.text(
      ...
      config.pageWidth / 2,
      30,
      { align: "center", maxWidth: config.contentWidth },
    );
    ...
    const participantData = [
      ...
      `Contato de Emergência: ${user.emergencyContact ?? "Não informado"}`,
    ];

    participantData.forEach((text, _) => {
      currentY = checkPageBreak(currentY, 10);
      doc.text(text, config.margin, currentY, {
    ...
    const imageY = currentY; // Pequeno espaçamento abaixo do texto
    ...
    const fileName = `termo_${user.name
      ?.trim()
      .normalize("NFD")
      .replace(/[\u0300-\u036f]/g, "")
      .replace(/\s+/g, "_")
      .replace(/[^\w\-\.]/g, "")
      .toLowerCase()}_${topNumber}.pdf`;
