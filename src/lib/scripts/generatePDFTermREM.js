 ...
 * @param {{name: string , cpf: string , phone: string , email: string  }} otherUser
 * @param { {topNumber: number, organization: string, initialDate: string}} event
 */
async function sendDocumentToAutentique(buffer, user, otherUser, event) {
  const action = "SIGN";
  const docName = `${user.name.replaceAll(" ", "_")}_e_${otherUser.name.replaceAll(" ", "_")}_top_${event.topNumber}_${event.organization.replaceAll(" ", "_")}_termo`;
  const docMessage = `Olá, ${user.name} e ${otherUser.name}. Assine o termo de responsabilidade para o REM#${event.topNumber} - ${event.organization}`;
  ...
  const operations = JSON.stringify({
    ...
    variables: {
      ...
      signers: [
        ...
        {
          phone: otherUser.phone.startsWith("+")
            ? otherUser.phone
            : `+${otherUser.phone}`,
          delivery_method: "DELIVERY_METHOD_WHATSAPP",
          action,
...
async function getConfirmedUsers() {
  ...
  return registersWithoutDocumentId.map((register) => ({
    ...
    spouseEmail: register.spouseEmail,
    spouseBirthDate: register.spouseBirthDate,
...
// * @param {{name: string | null, cpf: string | null, address: string | null, birthDate: string | null, phone: string | null, email: string | null, emergencyContact: string | null, topNumber: string | null  }} user
/**
 * @param {{name: string | null, cpf: string | null, address: string | null, birthDate: string | null, phone: string | null, email: string | null, emergencyContact: string | null, topNumber: string | null, spouseName: string | null, spouseCPF: string | null, spousePhoneNumber: string | null, spouseEmail: string | null, spouseBirthDate: string | null }} user
...
async function createUserPDF(user, topNumber) {
  ...
  doc.setFont("helvetica", "bold");
  doc.text("DADOS DO PRIMEIRO PARTICIPANTE:", margin, currentY);
  currentY += 10;
  ...
  // Espaçamento entre os dois participantes
  currentY += 10;

  doc.setFont("helvetica", "bold");
  doc.text("DADOS DO SEGUNDO PARTICIPANTE:", margin, currentY);
  currentY += 10;

  const spouseData = [
    { label: "Nome", value: user.spouseName ?? "Não informado" },
    {
      label: "CPF",
      value: user.spouseCPF
        ? mask(user.spouseCPF, "999.999.999-99")
        : "Não informado",
    },
    {
      label: "Data de Nascimento",
      value: user.spouseBirthDate
        ? format(new Date(user.spouseBirthDate), "dd/MM/yyyy")
        : "Não informado",
    },
    {
      label: "Telefone",
      value: user.spousePhoneNumber
        ? mask(user.spousePhoneNumber, "+99(99) 99999-9999")
        : "Não informado",
    },
    { label: "E-mail", value: user.spouseEmail ?? "Não informado" },
  ];

  spouseData.forEach(({ label, value }) => {
    // Write label in bold
    doc.setFont("helvetica", "bold");
    doc.text(`${label}: `, margin, currentY);
  ...
  doc.setFont("helvetica", "bold");
  doc.text(
    "Endereço e Contato de Emergência dos Participantes:",
    margin,
    currentY,
  );
  currentY += 7;
  ...
  comumnInfo.forEach(({ label, value }) => {
    currentY += 5;
  ...
  currentY = createBoxedTitle(
    ...
    [
      `O acima qualificado 'PARTICIPANTE' declara para todos os fins que entende e confirma que o seu comparecimento e participação no evento Reto de Empoderamento Matrimonial (REM), criado pela LEGENDARIOS INTERNACIONAL LLC, cuja execução foi devidamente licenciada à Sede VALE DAONÇA LTDA (aqui designadas, em conjunto, como 'LEGENDARIOS'), 'EVENTO' a ser realizado na pista VALE ONÇA TRACK ${topNumber}, na cidade de DOURADOS/MS, estado MS no Brasil, é baseado na sua vontade e desejo voluntário.`,
  ...
  currentY = createBoxedTitle(
    ...
    [
      ...
      "i. Reto de Empoderamento Matrimonial - REM;",
  ...
  currentY = createBoxedTitle(
    ...
    [
      "O PARTICIPANTE declara estar ciente e de acordo com a coleta, uso e armazenamento de seus dados pessoais para fins relacionados à sua inscrição, participação e comunicação referente ao evento Reto de Empoderamento Matrimonial (REM). As informações fornecidas poderão incluir, mas não se limitam a, nome, contato, documento de identificação e demais dados necessários para a organização do evento.",
