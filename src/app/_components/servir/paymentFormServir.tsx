...
import { createWhatsappGroupLinkServir } from "@/lib/whatsapp";
import { env } from "@/env";
...
export default function PaymentFormServir() {
  ...
  const onPay = async (charge: {
    ...
    if (charge.status === "COMPLETED") {
      ...
      resetValues();
      window.open(
        createWhatsappGroupLinkServir(
          data?.celular,
          eventRegister.linkWhatsappGrupoServir,
        ),
        "_blank",
      );
  ...
  const newCharge = async () => {
    ...
    const payload = {
      ...
      value: Math.floor(amount * 100), // in cents
      comment: `SERVIR TOP#${eventRegister.topNumero} - ${cpf}`,
      customer: {
        name: data?.nome ?? "",
        email: data?.email ?? "",
        taxID: data?.cpf ?? "",
