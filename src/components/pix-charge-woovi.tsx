...
export type PixChargeWooviProps = {
  charge: {
    brCode: string;
    identifier: string;
  } | null;
...
export const PixChargeWoovi = ({
  ...
  return (
    <div className="flex h-full w-full flex-col items-center justify-center rounded-md bg-background px-8 py-6">
      ...
      <div className="my-4 flex max-w-min items-center justify-center rounded-md border border-input p-2 shadow-md">
        <QRCodeSVG size={140} value={charge?.brCode} includeMargin={false} />
      </div>
      ...
      <div className="mt-6 flex flex-col items-center justify-center gap-6 text-center">
        ...
        <div className="flex flex-col gap-1">
          ...
          <p className="">Legendários MS - VALE DA ONCA</p>
        </div>

        <div className="flex flex-col gap-1">
          <h2 className="font-medium text-primary">Identificador</h2>
          <p className="leading-6 ">{charge?.identifier} 978189273182938</p>
        </div>
      </div>
      <div className="mt-4 flex flex-col gap-1 text-center">
        <h2 className="text-xs font-medium text-primary">Entre em contato</h2>
        <p className="text-xs">
          Pagamento Seguro e Criptografado pela <b>woovi</b>
        </p>
      </div>
    </div>
  );
